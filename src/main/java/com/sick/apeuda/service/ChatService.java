...
public class ChatService {
    ...
    // 채팅방에 입장한 세션 추가
    public void addSessionAndHandleEnter(String roomId, WebSocketSession session, ChatMsgDto chatMsgDto) {
        ChatRoomResDto room = findRoomById(roomId);
        if (room != null) {
            room.getSessions().add(session); // 채팅방에 입장한 세션 추가
            if (chatMsgDto.getSender() != null) { // 채팅방에 입장한 사용자가 있으면
                chatMsgDto.setMessage(chatMsgDto.getSender() + "님이 입장했습니다.");
                sendMessageToAll(roomId, chatMsgDto); // 채팅방에 입장 메시지 전송
            }
